...
class CompanySearchSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CompanySearchHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CompanySearchHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CompanySearchFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CompanySearchFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CompanySearchUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CompanySearchHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CompanySearchHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CompanySearchHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CompanySearchError($op . "_allow",
                "CompanySearchSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CompanySearchHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CompanySearchError("graphql_error",
                "CompanySearchSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function NearPoint($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_near_point === null) {
                $this->_near_point = new NearPointEntity($this, null);
            }
            return $this->_near_point;
        }
        return new NearPointEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CompanySearchSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
