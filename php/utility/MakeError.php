...
class CompanySearchMakeError
{
    public static function call(?CompanySearchContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CompanySearchContext([], null);
        }
        $op = $ctx->op ?? new CompanySearchOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CompanySearchResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CompanySearchError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CompanySearchError('', $msg, $ctx);
