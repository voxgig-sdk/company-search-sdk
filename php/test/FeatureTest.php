...
// CompanySearch SDK feature test
//
...
class FtCtrl extends CompanySearchControl
{
...
class FtClient
{
    ...
    private CompanySearchUtility $utility;

    public function __construct(CompanySearchUtility $utility, array $options)
    {
    ...
    public function get_utility(): CompanySearchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CompanySearchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CompanySearchContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CompanySearchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CompanySearchConfig::make_feature($name);
    ...
    private static function buildUrl(CompanySearchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CompanySearchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CompanySearchResponse($fetched);
        }
    ...
    private function fail(CompanySearchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CompanySearchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
