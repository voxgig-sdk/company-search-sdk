...
// CompanySearch SDK NearPoint entity
...
class NearPointEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new NearPointEntity($this->_client, $opts);
    }

    /**
     * @param NearPoint|array $args NearPoint data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CompanySearchHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return NearPoint|array The current NearPoint data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of NearPoint fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CompanySearchHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of NearPoint fields).
     */
    ...
    /**
     * List NearPoint items matching the given filter.
     *
     * @param NearPointListMatch|array|null $reqmatch Match filter (any subset
     *   of NearPoint fields) as an assoc-array; NearPointListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return NearPoint[]|array A list of NearPoint items as assoc-arrays at
     *   the SDK boundary; throws CompanySearchError on failure (item-5 convention).
     */
