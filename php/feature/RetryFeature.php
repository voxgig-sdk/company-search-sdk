...
// CompanySearch SDK retry feature
...
class CompanySearchRetryFeature extends CompanySearchBaseFeature
{
    ...
    public function init(CompanySearchContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CompanySearchContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CompanySearchContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CompanySearchContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
