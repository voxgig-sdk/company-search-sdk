...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.COMPANYSEARCH_TEST_LIVE ||
    'TRUE' === process.env.COMPANYSEARCH_TEST_OVERRIDE
  ) {
  ...
  m.COMPANYSEARCH_TEST_EXPLAIN = process.env.COMPANYSEARCH_TEST_EXPLAIN || m.COMPANYSEARCH_TEST_EXPLAIN
