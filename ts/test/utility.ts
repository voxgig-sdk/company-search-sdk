...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.COMPANY_SEARCH_TEST_LIVE ||
    'TRUE' === process.env.COMPANY_SEARCH_TEST_OVERRIDE
  ) {
  ...
  m.COMPANY_SEARCH_TEST_EXPLAIN = process.env.COMPANY_SEARCH_TEST_EXPLAIN || m.COMPANY_SEARCH_TEST_EXPLAIN
