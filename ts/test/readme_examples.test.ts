...
//     (\`new CompanySearchSDK(...)\` / \`CompanySearchSDK.test(...)\` become
//     \`CompanySearchSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CompanySearchSDK'
...
const TEST_SEED = {"entity":{"near_point":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CompanySearchSDK(...)\` and any
// \`CompanySearchSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CompanySearchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CompanySearchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
