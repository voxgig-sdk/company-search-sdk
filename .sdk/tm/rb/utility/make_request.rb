...
module CompanySearchUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CompanySearchResponse.new({})
    ...
    if fetch_err
      ...
      response = CompanySearchResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CompanySearchResponse.new(fetched)
    else
