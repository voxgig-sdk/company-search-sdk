...
// CompanySearch SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class CompanySearchMakePoint
{
    public static function call(CompanySearchContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof CompanySearchError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = CompanySearchHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = CompanySearchHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
