...
The Golang SDK for the CompanySearch API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/company-search-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/company-search-sdk=../path/to/github.com/voxgig-sdk/company-search-sdk
```
...
```go
    result, err := client.NearPoint(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
...
```
github.com/voxgig-sdk/company-search-sdk/
...
The root package (`github.com/voxgig-sdk/company-search-sdk`) re-exports everything needed
