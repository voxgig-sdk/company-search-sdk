# CompanySearch SDK test runner

from __future__ import annotations
import os
import json

from companysearch_sdk.utility.voxgig_struct import voxgig_struct as vs


class CompanySearchTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CompanySearchTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CompanySearchTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CompanySearchTestRunner.getenv("COMPANY_SEARCH_TEST_LIVE")
        override = CompanySearchTestRunner.getenv("COMPANY_SEARCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CompanySearchTestRunner.getenv(key)
        ...
        explain = CompanySearchTestRunner.getenv("COMPANY_SEARCH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CompanySearchTestRunner._test_control is not None:
            return CompanySearchTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CompanySearchTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CompanySearchTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CompanySearchTestRunner.load_test_control()
...
def load_env_local():
    CompanySearchTestRunner.load_env_local()


def env_override(m):
    return CompanySearchTestRunner.env_override(m)


def entity_data(v):
    return CompanySearchTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CompanySearchTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CompanySearchTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CompanySearchTestRunner.load_test_control()


def live_delay_ms():
    return CompanySearchTestRunner.live_delay_ms()
