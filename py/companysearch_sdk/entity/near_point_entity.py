...
from companysearch_sdk.companysearch_types import (
    NearPoint,
    NearPointListMatch,
)
...
class NearPointEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NearPointEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[NearPoint]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.NearPoint().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
