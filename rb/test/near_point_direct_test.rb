...
class NearPointDirectTest < Minitest::Test
  def test_direct_list_near_point
    setup = near_point_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-near_point", setup[:live] ? "live" : "unit")
    if _should_skip
...
def near_point_direct_setup(mockres)
