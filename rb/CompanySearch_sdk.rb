...
require_relative 'CompanySearch_types'
...
class CompanySearchSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CompanySearchHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CompanySearchHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CompanySearchFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CompanySearchUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CompanySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CompanySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CompanySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CompanySearchHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CompanySearchError.new(
        "graphql_error", "CompanySearchSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CompanySearchSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
