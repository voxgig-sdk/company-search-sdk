# CompanySearch Ruby SDK



The Ruby SDK for the CompanySearch API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `CompanySearchError` (a `StandardError` subclass), so wrap
...
| `finances` | `Hash` | Bilans financiers par année |
...
```
rb/
├── CompanySearch_sdk.rb       -- Main SDK module
