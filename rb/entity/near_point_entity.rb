...
class NearPointEntity
  ...
  def make
    opts = @_entopts.dup
    NearPointEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CompanySearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [NearPoint, Hash] the current NearPoint data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CompanySearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of NearPoint fields)
  def match_get
  ...
  #
  # @param reqmatch [NearPointListMatch, Hash, nil] match filter (any subset of
  #   NearPoint fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<NearPoint>, Array] the matching NearPoint items; raises CompanySearchError on failure
