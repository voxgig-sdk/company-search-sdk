...
boru-driven command-line client **and** interactive REPL for the CompanySearch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/company-search-cli)
make build

# 2. See usage (words, entities, env vars)
./company-search-cli --help

# 3. Provide credentials once, via the environment
export COMPANY_SEARCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./company-search-cli list near_point
./company-search-cli list search

# 5. Override the API base URL for a single call
COMPANY_SEARCH_BASE=https://api.example.com ./company-search-cli list near_point

# 6. No arguments -> interactive REPL
./company-search-cli
company-search> list near_point
company-search> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/company-search-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/company-search-cli list near_point
   ```
...
Run with no arguments to open a REPL (prompt `company-search>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `near_point`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/company-search-cli`. |
...
(`./company-search-cli <expr>`) and interactively (the REPL), and expressions compose the
