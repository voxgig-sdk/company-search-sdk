...
The Lua SDK for the CompanySearch API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:NearPoint(nil):list(nil, nil)
...
local result, err = client:CompanySearch(nil):load(
  { id = "test01" }, nil
)
