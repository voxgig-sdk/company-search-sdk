...
local NearPointEntity = {}
NearPointEntity.__index = NearPointEntity


function NearPointEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NearPointEntity)
  self._name = "near_point"
  self._client = client
...
function NearPointEntity:get_name()
  return self._name
end


function NearPointEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NearPointEntity.new(self._client, opts)
end
...
function NearPointEntity:mark_deleted()
  self._deleted = true
end


function NearPointEntity:deleted()
  return true == self._deleted
end


function NearPointEntity:data_set(args)
  if args ~= nil then
...
function NearPointEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NearPointEntity:match_set(args)
  if args ~= nil then
...
function NearPointEntity:match_get()
...
function NearPointEntity:stream(action, args, callopts)
...
function NearPointEntity:_run_op(ctx, post_done)
  local utility = self._utility
